...
def main():
    ...
    allGoogleEvents = googleCalendar.get_google_calendar(
        googleCheckBegin.isoformat()+'Z', 10, GoogleCalendarPrefix)
    ...
    setGoogleEvents = set((event.start, event.end, event.summary)
                          for event in outlookGoogleEvents)
    newGoogleEvents = [event for event in originOutlookEvents if (
        event.start, event.end, event.summary) not in setGoogleEvents]
